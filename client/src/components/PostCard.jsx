import { BadgeCheck, Heart, MessageCircle, Share2, Edit2, Trash2, MoreVertical, X, Save, Send, Repeat2, Sparkles, Lightbulb, FileText, Wand2, TrendingUp } from "lucide-react";
  ...
  return (
    <div className="bg-white rounded-xl shadow p-4 space-y-4 w-full max-w-2xl">
      ...
      <div className="flex items-center gap-4 text-gray-600 text-sm pt-2 border-t border-gray-300">
        <div className="flex items-center gap-2">
          <Heart
            className={`w-4 h-4 cursor-pointer transition-colors ${
              (Array.isArray(likes) ? likes.some(like => typeof like === 'object' ? like._id === currentUser._id : like === currentUser._id) : false) && "text-red-500 fill-red-500"
            }`}
            onClick={handleLike}
          />
          <span>{Array.isArray(likes) ? likes.length : 0}</span>
          {/* Show profile pictures of likers */}
        ...
        >
          <Repeat2 className={`w-4 h-4 ${hasReposted ? 'text-green-600' : ''}`} />
        ...
        {/* Waves - Analytics Button with Views Count (Only for own posts) */}
        {isOwnPost && (
          <div 
