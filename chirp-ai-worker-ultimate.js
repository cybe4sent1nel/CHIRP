...
/**
 * Format AI response to clean markdown without excessive asterisks
 * Removes unnecessary bold formatting while keeping structure
 */
function formatCleanResponse(text) {
  ...
  // Convert **Title:** patterns to proper headers
  cleaned = cleaned.replace(/^\*\*([^*]+):\*\*/gm, '## $1\n');
  
  // Remove bold around single words not in lists
...
async function processAIRequest(body, env) {
  const {
    ...
    tools = true,
    maxTokens = 1024,
    modelHint = 'auto', // NEW: Allow client to hint at model preference
  } = body;
  ...
  try {
    ...
    // Use modelHint if provided, otherwise auto-detect
    let model;
    if (modelHint === 'quality') {
      model = MODELS.TEXT.quality; // Llama 3.3 70B
    } else if (modelHint === 'fast') {
      model = MODELS.TEXT.fast; // Mistral 7B
    } else if (modelHint === 'reasoning') {
      model = MODELS.TEXT.reasoning; // DeepSeek R1
    } else if (modelHint === 'code') {
      model = MODELS.TEXT.code; // DeepSeek R1
    } else {
      // Auto-detect based on prompt
      model = selectModel(prompt, { type });
    }
    ...
    log('INFO', 'Calling AI model', { model, modelHint, messageCount: messages.length, maxTokens });
    ...
    const cleanedText = formatCleanResponse(generatedText);
    addToHistory(session, 'assistant', cleanedText);
